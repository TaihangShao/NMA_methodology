...
ltBC1 <- lifeTable(che, timeColumn = "rectime2", censColumn = "censrec")
...
modFP2_2 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^(0)*lnTime) + I(Time^0*lnTime*lnTime) + offset(log(timedelta)), family=binomial(link=cloglog), data=ltHaz)
...
dfsurv_lthaz<-as.data.frame(array(dim=c(71,21)))
...
f_surv2= ggplot() +
  ...
  expand_limits(y=c(0,1),x=c(0,4.5)) + 
  facet_wrap(~Model,nrow=4)+
  scale_x_continuous(breaks = c(seq(from=0, to=4.5,by = 0.5))) +
...
ggsave("fig_surv_che_os_fit.png",f_surv2,width = 12,height = 8,dpi = 1200)
...
dfsurv<-as.data.frame(array(dim=c(348,21)))
...
write.csv(dfsurv,"surv.csv")


##当某模型不能拟合时
modnames <- list("exp","weibull","gamma","lnorm","gompertz","llogis","gengamma","genf","FP1-1","FP1-2",
                 "FP2-1","FP2-2","RCS1","RCS2","RP-hazard-1","RP-hazard-2","RP-odds-1","RP-odds-2","RP-normal-1")


lthaz_plot<-data.frame(ltHaz$Time,ltHaz[,11:29])
dfsurv_lthaz<-as.data.frame(array(dim=c(length(ltHaz[,1]),20)))
colnames(dfsurv_lthaz)<-c("Time",modnames)
dfsurv_lthaz$Time<-lthaz_plot$ltHaz.Time
for (i in 1:19) {
  temp_dfhaz<-data.frame(lthaz_plot$ltHaz.Time,lthaz_plot[,i+1])
...
f_surv2= ggplot() +
  ...
  geom_line(data = ltHaz,aes(x=Time,y=surv),size=0.8)+
  scale_color_discrete(name="Model")+
  expand_limits(y=c(0,1),x=c(0,2.5)) + 
  facet_wrap(~Model,nrow=4)+
  scale_x_continuous(breaks = c(seq(from=0, to=2.5,by = 0.5))) +
  ylab("Overall survival") +
  xlab("Time(years)") +
  guides(color = guide_legend(ncol = 1))  +
  theme(legend.position = "bottom") + 
...
ggsave("fig_surv_fit.png",f_surv2,width = 12,height = 8,dpi = 1200)


##################------------surv rate calculate  ###  dfhazest   ###---------------##############
#calculate survival over time
dfhar<-t(dfHazEst)
dfhar<-data.frame(dfhar[,1:7],dfhar[,9:20])
colnames(dfhar) <- modnames
dfhar <- cbind(data.frame(Newtime$Time), dfhar)
dfhar <- rename(dfhar,"Time"="Newtime.Time")
...
write.csv(dfsurv,"surv_data_20y.csv")
