...
# Working diectory
path <- "C:/Users/MAC/Desktop/NMA_NSLCL"
setwd(path)

# Load IPD data
data <- read.csv("RP_PFSXIU.csv")
...
num_patients <- c(492,679,578,1094,616,550)####
...
data$trt3 <- 0
data$trt3[data$txCode==3 & data$studyCode==2] <- 1
data$trt3[data$txCode==3 & data$studyCode==3] <- 1

data$trt4 <- 0
data$trt4[data$txCode==3 & data$studyCode==4] <- 1
...
bugs_data <- list(d=data$event, trt2=data$trt2, trt3=data$trt3, trt4=data$trt4, trt5=data$trt5, trt6=data$trt6, 
                  ...
                  trt6lnt=data$trt6lnt, trt7lnt=data$trt7lnt,
                  u0=data$u0, u1n=data$u1n, u2n=data$u2n, du0=data$du0,
                  du1n=data$du1n, du2n=data$du2n, Ntrials=num_studies, offset=offset,
                  k0=knots$k1[5], k1=knots$k2[5], k2=knots$k3[5], k3=knots$k4[5],
                  meanv0=df1$mean[5], sdv0=df1$sd[5], v1w0=df2$v1u0[5], w0w0=df2$u0u0[5],
                  meanw1=df3$mean[5], sdw1=df3$sd[5], v2w0=df4$v2u0[5], v2w1n=df4$v2u1n[5],
                  w1nw1n=df4$u1nu1n[5], meanw2=df5$mean[5], sdw2=df5$sd[5],
                  nt=nt)
...
bugs.object<-bugs(data=bugs_data, inits=inits, 
                  ...
                  model.file="model_PFSXIU.txt", clearWD=F, 
...
write.csv(results, "results_PFSXIU.csv")